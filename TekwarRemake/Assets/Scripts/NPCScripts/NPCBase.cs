...
public abstract class NPCBase : PlayerObj
{
	...
	void Awake()
	{
		...
		flee = false;
		alert = 0;
		nav_npc = GetComponent<NavMeshAgent>();
					...
					if(hit.transform == player)
					{
						...
						if(waittime_sc == 0)
						{
							...
							if(alert_typ != NpcAlert.None)
								audio.PlayOneShot(alert_snds[Random.Range(0,alert_snds.Length)]);
						}
						if(hweapon)
						{
							//Shoot waittime
								...
								if(angle < LOS_SHOOT)
								{
									...
									if(proj_shot)
									{
										...
										((GameObject)GameObject.Instantiate(GameBase.inst.pr_enemy,
											proj_point.position,proj_point.rotation)).rigidbody.velocity =
	...
	void DisableNPC()
	{
		if(death_typ != NpcDeath.None)
			audio.PlayOneShot(death_snds[Random.Range(0,death_snds.Length)]);
		enabled = false;

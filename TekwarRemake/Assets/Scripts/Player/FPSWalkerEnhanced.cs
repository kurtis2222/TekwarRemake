...
public class FPSWalkerEnhanced: MonoBehaviour {
    ...
    void FixedUpdate() {
		...
		{
			if(transform.parent.collider is MeshCollider)
			{
				if(!transform.parent.collider.bounds.Contains(transform.position))
					transform.parent = null;
			}
			else
			{
				if(!transform.parent.collider.bounds.Contains(transform.position-elev_diff))
					transform.parent = null;
			}
		}
	next:

...
public class MainScript : PlayerObj
{
	...
	void Update()
	{
		...
		else if(Input.GetButtonDown("Music"))
		{
			...
			{
				if(isplaying) main_cam.audio.Stop();
				else main_cam.audio.Play();
				isplaying = !isplaying;
			}
		}
	...
	public void PlayMusic(int idx)
	{
		...
		main_cam.audio.clip = GameBase.inst.GetMusic(idx);
		if(isplaying) main_cam.audio.Play();
	}
	
	void DMBlink()
	{
		if(Random.Range(0, 4) == 0)
			audio.PlayOneShot(hit_snd[Random.Range(0,hit_snd.Length)]);
	}
	
	void CosBlink()
	{
		audio.PlayOneShot(item_snd);
	}
